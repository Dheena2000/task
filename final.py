...
def debug_html_differences():
    """Debug and print the differences between the old and new HTML files."""
    with open(HTML_OLD, "r", encoding="utf-8", errors="replace") as f1, open(HTML_NEW, "r", encoding="utf-8", errors="replace") as f2:
        lines1 = f1.readlines()
        lines2 = f2.readlines()
        ...
        for i, (line1, line2) in enumerate(zip(lines1, lines2)):
            if line1 != line2:
                differences.append(f"Difference found on line {i + 1}")
                differences.append("Old: " + line1.strip())
                differences.append("New: " + line2.strip())
...
if not os.path.exists(SCREENSHOT_OLD):
    ...
    with open(HTML_OLD, "rb") as f1, open(HTML_NEW, "rb") as f2:
        htmls_equal = f1.read().hex() == f2.read().hex()


    if not (screenshots_equal and htmls_equal):
        print(debug_html_differences())  # Print the differences
        send_alert()
